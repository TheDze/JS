...
<body>
    ...
    <?php
        ...
        if(mysqli_errno($baza)){
            ...
            for($i=1;$i<=5;$i++){
                ...
                for($ii=1;$ii<=9;$ii++){
                    $wynik=mysqli_query($baza,"SELECT nazwa,sala FROM plan WHERE nr=$ii AND dzien=$i;");
                    $row=mysqli_fetch_array($wynik);
                    echo "<td>$row[0]<br>$row[1]";
                }
            }
        mysqli_close($baza);
    ?>
    ...
    <div id='butony'>
        <a href="http://localhost:8080/zadanko/2.php">
        <button>Zmień / Usuń</button></a>
    </div>
</body>
</html>